...
* **Slack Connect channels.** By default, Claude is off in channels shared with another company. A mention there gets no reply and no message explaining why. Your admin can [turn Claude on for Slack Connect channels](/docs/claude-tag/admins/restrict-access#slack-connect-channels). Claude then replies without your organization's memory or workspace instructions, and with only the tools your admin turned on for Slack Connect channels.
